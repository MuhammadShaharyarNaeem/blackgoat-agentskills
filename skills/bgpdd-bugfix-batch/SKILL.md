...
- **Phase transitions at the batch level are emitted, not recalled (convention #9), the same obligation `bgpdd-bugfix` § 1 states for its own phases**: before deciding what to do next, run `python {PLUGIN_ROOT}/pipeline-tools/scripts/pipeline_driver.py --root {batch-root} --lane batch --milestone "{batch-slug}" --json`. It reads `orchestrator-state.json` and prints the next open bug's own driver invocation, or — once every bug is terminal — the close gate, or — once that gate has PASSed — that the batch is complete (exit 3). Each bug's OWN phase inside its worktree stays driven by `bgpdd-bugfix` § 1's `--root {bugfix-root} --lane bugfix` invocation, unchanged; the batch driver names it rather than substituting for it.

## 2. Execution Workflow

### Phase 0: Batch list (main session)
Agree the bug list with the user; pick `{batch-slug}` and a `{bug-slug}` each; **N ≤ 5** (§3). No report is written yet — its only home is `{bugfix-root}` *inside* a worktree that does not exist until Phase 1. **Initialize the batch state** — the only durable record of the bug list until every worktree exists: `python {PLUGIN_ROOT}/pipeline-tools/scripts/update_state.py --state {batch-root}/orchestrator-state.json --init --project-name "{batch-slug}" --set-pipeline bgpdd-bugfix-batch --set-artifact "bugs=<bug-slug-1>,<bug-slug-2>,..."`.

### Phase 1: Isolation, then intake
Per bug: `git worktree add <path> -b fix/{bug-slug} <base>`, `<base>` read from the repo (`pipeline-skeleton` § *Branch close*). **Record it in the batch state**: `python {PLUGIN_ROOT}/pipeline-tools/scripts/update_state.py --state {batch-root}/orchestrator-state.json --set-artifact "bug:{bug-slug}:root=<bugfix-root>" --set-artifact "bug:{bug-slug}:worktree=<path>" --set-artifact "bug:{bug-slug}:status=OPEN" --milestone "{bug-slug}"` — the fields `check_batch_close.py` and the batch driver read (Rules §3: each of these calls is also a game-tape event). **Each worktree is then its own single lane to the hook** — `guard_action.py`'s detector is `cwd`-rooted and globs `<cwd>/.docs/bugfix/*/bug-report.md` (rationale § 2). `.docs/bugfix-batch/` matches no detector, so `{batch-root}` arms no lane. Then, inside each worktree, `bgpdd-bugfix` § Phase 0 for that bug, gate included, **one bug at a time with the user** (Contract §1: interactive steps are never delegated).
...
2. Then, **one bug at a time, in §3's overlap order**: **(a) precondition, mechanical (convention #9)** — `{PLUGIN_ROOT}/pipeline-tools/scripts/check_ledger.py --ledger <that bug's root>/gates.jsonl` exits 0 **and** that ledger's latest `check_commit_gate.py` record is a `PASS` carrying `--commit`; **(b)** commit that bug's `.docs/bugfix/{bug-slug}/` on its own branch — the gate exempts `.docs/` from `--verify-tree`, so its evidence is untracked and Phase 4's `git worktree remove` would delete it, and `guard_action.py` rule 1's closed-lane carve-out permits this commit once (a) holds; **(c)** merge `fix/{bug-slug}` into the base; **(d)** record it — `update_state.py --state {batch-root}/orchestrator-state.json --set-artifact "bug:{bug-slug}:status=MERGED" --milestone "{bug-slug}"` (Rules §3's game-tape bullet).
3. An overlap or a conflict routes that bug: **rebase its branch on the merged base, re-run its § Phase 4 GREEN and `check_red_green.py` there**, re-commit its evidence, then merge. Never re-enter step 1 (the fix is committed — the gate answers `already_committed`) and never hand-resolve a conflict: a rebase that cannot replay cleanly is a HALT (§4) — its `bug:{bug-slug}:status` stays `OPEN`, which is exactly what keeps Phase 4's close gate from treating it as done.

### Phase 4: Batch close
1. `batch.md`'s `## Final` table: a row per bug citing its terminal status, its commit sha and ledger record (MERGED — step 2(a) verified) or its escalated-evidence path (DROPPED-PLAN — Phase 2).
2. `git worktree remove` for every bug that still has one (a MERGED bug's evidence is committed, per step 2(b); a DROPPED-PLAN bug's evidence was copied out, per Phase 2).
3. **Gate, not prose (convention #9)**: `python {PLUGIN_ROOT}/pipeline-tools/scripts/check_batch_close.py --state {batch-root}/orchestrator-state.json --batch-md {batch-root}/batch.md --repo <the target repo root> --milestone "{batch-slug}" --ledger {batch-root}/gates.jsonl`. Verifies every bug in the state's `bugs` list is terminal (`MERGED` — with its own ledger's `check_commit_gate.py` PASS carrying `--commit` — or `DROPPED-PLAN`), every bug's recorded worktree path no longer exists on disk, `{batch-root}/gates.jsonl`'s own hash chain is intact, and `batch.md`'s `## Final` table names every bug with a status-appropriate row. Exit 0 = closed, and is itself the write §1 names for `{batch-root}/gates.jsonl` (no other script appends there). Exit 1 = **BLOCK**; the JSON names the open bug, the leftover worktree, the broken chain or the table mismatch — fix it and re-run. Never hand-close a batch this gate has not PASSed.
...
- **Ledger**: per-bug gates carry `--ledger {bugfix-root}/gates.jsonl --milestone "{bug-slug}"`; `{batch-root}/gates.jsonl` holds only `check_batch_close.py`'s own runs (Phase 4 step 3) — `check_ledger.py` itself writes nothing (`pipeline-tools/SKILL.md`: it is the chain's reader, "the subject under test, not an append target"), so the batch's own close gate is the sole writer this file ever gets.
- **Game tape — one bullet per per-bug status update (deliberately tighter than the skeleton's once-per-run cadence, convention #8)**: every `update_state.py --set-artifact "bug:{bug-slug}:status=..."` call — Phase 1's `OPEN`, Phase 2's `DROPPED-PLAN`, Phase 3's `MERGED` — is a state persistence (Orchestrator Contract §4) and gets one `{batch-root}/game-tape.md` bullet naming the bug, the old status and the new one, not only a summary written at Phase 4's close.
