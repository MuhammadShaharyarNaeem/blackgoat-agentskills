...
2. Reply to the Orchestrator (your Subagent Manager) using strict XML handoff tags: `<handoff><status>COMPLETE</status><artifact>path/to/file.md</artifact><blockers>None</blockers></handoff>`. When `<status>` is `PARTIAL` or `BLOCKED`, `<blockers>` must also carry a `blocked_on: <category> — <one-line reason>` line (category: environment, credentials, dependency, spec, or defect) — `check_handoff.py` gates on it, and it is what lets the Orchestrator's `check_redelegation.py` decide whether re-delegating you again would just re-discover the same wall. If your unit already carries a standing halt, returning `COMPLETE` does not clear it: `check_redelegation.py` never clears a halt on any result, for any agent — clearing is the user's own recorded word for what changed in the world (`update_state.py --clear-halt ... --reason ...`), not a re-run passing.
...
Never run an unbounded command. Every shell command or long-running tool call MUST carry an explicit timeout of at most **4 minutes (240s)** — via your runtime's timeout parameter or a wrapper — so a hung process is killed automatically. The plugin's own wrapper is `python {PLUGIN_ROOT}/pipeline-tools/scripts/run_quiet.py --log <path> --timeout 240 -- <cmd>` (`--timeout` is seconds and already defaults to 240; exit 124 with a `TIMEOUT:` header is a harness result to escalate, never a test FAIL). A raw `dotnet build`/`dotnet test`/`npm test`/`pytest`/`npx playwright test` (and the other runners `guard_action.py` names, even behind an env-var prefix or a `bash -c`/`cmd /c` wrapper) run directly by Bash while a lane is active is no longer just discouraged by this paragraph — it is mechanically denied (rule 5, `build_and_test_through_the_wrapper`) unless `run_quiet.py` is on the line. Where no Python 3 runtime is available, use your runtime's own timeout parameter or a shell wrapper — POSIX `timeout 240 <cmd>`; note `timeout` on PowerShell is a sleep, not a wrapper. If the bound kills a command: capture the partial stdout/stderr and do NOT re-run it unchanged. You may retry once with a change that plausibly ends the hang (smaller scope, filtered subset, incremental step), or — for a genuinely long operation (full build, package restore, full suite) — a single longer bound stated with its reason. A second timeout on the same operation is a blocker: stop and escalate via `<handoff>` with the command, the bound, and the partial output. Timeouts count toward your same-error circuit breaker.

**A `pipeline-tools` script is invoked, never read.** `python {PLUGIN_ROOT}/pipeline-tools/scripts/<name>.py --help` is the complete contract — purpose, every flag, every exit code, and where a problem code surfaces — at a fraction of the cost of opening several hundred lines of source to learn how to call it. Opening the source to learn how to invoke it is exactly the token cost the script exists to remove (convention #9, refined here: the artifact that has to be *run* is the gate, not its own source). **Exception, stated explicitly:** when the task IS editing that script — the working tree is the plugin repository itself — reading it is authoring, not execution.
...
- **A claim's confidence may never exceed the scope of what you observed.** A finding is bounded by *what* you checked and *where*. Do not promote a scope-bounded negative — "not found in the files/dirs I searched" — into "does not exist" or "lives in some other system" without first running the cheapest search that would settle it (e.g. a repository-wide grep for the symbol). Do not promote a single-source or single-member positive — "true for the one caller/value I looked at" — into an authoritative named entity ("the X enum", "all the sibling fields behave this way") without reading its defining source and checking each member. When a claim must be stated more broadly than the observation grounding it, label the unverified span (`unverified — seen only in <scope>`) rather than asserting it. Widening the search before you conclude is a consumer-side duty at the moment of the claim; it is not a licence to cast every search wide by reflex.
