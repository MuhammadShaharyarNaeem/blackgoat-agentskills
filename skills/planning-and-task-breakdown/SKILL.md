...
- [ ] Every task's `Dependencies:` field is complete — stated as exact task numbers (or `None`) so the build order is unambiguous. (Exact file paths to create/modify live in `Named identifiers:`.)
