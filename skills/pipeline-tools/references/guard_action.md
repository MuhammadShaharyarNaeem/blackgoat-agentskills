...
Two more restraints converted from "should not" to "cannot" — the guard now enforces six, not four.

**Rule 5** denies a raw `dotnet build`/`dotnet test`/`npm test`/`npm run build`/`npm run test`/`pnpm test`/`yarn test`/`pytest`/`npx playwright test`/`npx vitest`/`npx jest` run through `Bash` while any lane is active, unless `run_quiet.py` is on the line — the mechanical form of the plugin's long-standing "run builds and tests through `run_quiet.py`" prose rule in `base-persona.md` § Command Timeout Discipline. Matching is on the command's LEADING TOKENS per shell segment (split on `;`/`&`/`|`/newline), not a substring search, so a runner name quoted inside a commit message or an `echo` never matches, and `npm run build:prod` (an unrecognized script name) stays allowed on purpose — the tuple match is exact, not a prefix on the script name.

A first round shipped with a real gap: `DOTNET_CLI_TELEMETRY_OPTOUT=1 dotnet test`, `env VAR=1 dotnet test`, and `bash -c "dotnet test"` all bypassed the rule by hiding the real executable behind an env-var-assignment prefix or a wrapping interpreter shell. The fix strips a leading `NAME=value` shell-variable-assignment or `env` prefix before matching, and recurses into an interpreter wrapper's inline command string when one opens the segment (`cmd /c`, `powershell -c`/`-Command`, `pwsh -c`/`-Command`, `bash -c`, `sh -c`) — closing both bypasses without changing the "leading tokens, not substring" shape that keeps quoted mentions and unrecognized script names allowed.

**Rule 6** denies `Task`/`Agent` while the active lane's own `orchestrator-state.json` carries a non-empty `state["halt"]` — the mechanical form of Orchestrator Contract §2's "halt after 3 consecutive attempts and request human intervention." `halt` is written by `check_redelegation.py` (via `update_state.py --set-halt`) on `halt_environment`/`halt_dependency`/`halt_repeated_blocker`/`halt_round_bound`. The refusal names the halted unit, its code, its reason, and the exact `update_state.py --clear-halt "<unit>" --reason "<what changed>" --ledger <path>` command that lifts it — with the lane's real state-file and ledger paths substituted in, so the user isn't left to reconstruct them. There is deliberately no auto-clear-on-PASS path: only a human running `--clear-halt` themselves removes `state["halt"]`, because a blocker in `environment`/`credentials`/`dependency` is about the unit's world, not about which agent's next handoff happened to look clean. Rule 6 reads from the SAME lane-detection result rule 1 already computed for the call, not a separate pass; a halt belonging to a lane outside the 12-hour freshness window is not armed, because the lane itself is not detected.

Self-test count: 88 → 100 — rule 5's eleven raw-runner denials plus its wrapped/outside-a-lane/quoted-argument allows, rule 5's env-assignment/`env`-prefix and `bash -c`-wrapper fix round (4 cases), and rule 6's halted/cleared/no-lane/stale-lane cases (4 cases).

## Rule 7 (`blackgoat_persona_is_hand_edited_only`) and the status-aware lane close (Unreleased)

**Rule 7** denies `Write`/`Edit`/`MultiEdit`, or a `Bash` mutation (BASH WRITES), whose target resolves to `agents/blackgoat.md` — always, lane or no lane, mirroring rule 4's unconditional scope. `agents/blackgoat.md` is CLAUDE.md convention #7's file — the human author's psychological profile — and no agent may edit it, not even Forge. The single carve-out is the sanctioned append to `## Part VIII: Problem & Solution Ledger`, and it requires BOTH halves: the environment variable `BLACKGOAT_ALLOW_PART_VIII_APPEND=1` in the hook's own environment, AND — checkable only for `Write`/`Edit`/`MultiEdit`, whose payload carries the before/after text — a checked PURE append landing after the Part VIII heading (old content unchanged, new content is old content plus a suffix). `Bash` gets no carve-out even with the env var set: a shell command's target is a path string, not a diff, so there is nothing cheap to check that `>>` was the only thing done to the file.
