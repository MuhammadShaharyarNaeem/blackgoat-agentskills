...
- **Handoff validation, bound to this pipeline's returns.** Contract §1 requires every returned `<handoff>` to be validated before you act on it. Save each to a file, then run `python {PLUGIN_ROOT}/pipeline-tools/scripts/tool_registry.py for --lane bgpdd-plan --phase '*'` and execute the `check_handoff` line it prints, on all of them: Rex (Phase 1 Step B, and each re-delegation round of its loop), Aria (Phase 2, and her Phase 2.5 revision round), Alex (Phase 3, and each Phase 3.5 re-delegation round). **`--since` is always passed**; add `--fix-round` on a remediation return. Exit 0 = proceed. Exit 1 = the agent's contract was not met — return to that same agent, counting the round its own loop's bound counts. Exit 2 = usage or an artifact defect — fix the invocation or the artifact, not the agent, before re-running.
- **Run log** (Contract §4's obligation, bound here; CLI contract in `{PLUGIN_ROOT}/pipeline-tools/SKILL.md`). Per delegation return — Rex (Phase 1 Step B), each Scout (Phase 2 split), Aria (Phase 2 and her one revision round), Alex (Phase 3 and each coverage-fix round): run `record_run` (same phase `*` listing as above) (`--model` is mandatory on a delegation record — exit 2 without it; name the tier the delegation actually ran at, not the one recommended; a non-Claude runtime id also needs `--tier <haiku|sonnet|opus|fable>`). Add `--rounds <this artifact's round number>` on a re-delegation, plus a token measurement — `--tokens-in`/`--tokens-out` (or `--from-json <the completion payload>`) from the runtime's completion notification, or `--tokens-unavailable "<runtime>: <reason>"` when it exposes none — which is now equally mandatory, and `--status` from the handoff. Into Phase 4's game tape: `summarize_run` (same phase `*` listing).
...
- **Workflow**:
  ...
  5. **Tier-1 drift (mechanical, convention #9)** — the consumer half of the provenance contract `bgpdd-discovery` §1 states: run `python {PLUGIN_ROOT}/pipeline-tools/scripts/tool_registry.py for --lane bgpdd-plan --phase pre-flight` and execute the `check_tier1_provenance` line it prints. Drift warns, never halts: relay the artifact, its stamped sha and current HEAD, and let the user decide whether to plan against this map or re-run `/bgpdd-discovery` first. `--allow-drift "<reason>"` records their explicit word — never supply it yourself. When `check_tier1_provenance` reports drift, also run that listing's `tier1_staleness` line and paste its table in place of the bare drift warning — it tells the user WHICH docs the drift actually touches, not just that HEAD moved, which is what the user needs to decide "plan against this map" vs "re-run discovery for this feature only." Most rows will carry `stamp_source: inherited:overview.md` — that is expected, not a gap. Then proceed to Phase 1.
...
- **Step B — Spec synthesis (delegate isolated Rex)**:
  ...
  7. Read Rex's returned handoff (validated per §1). If he flags unresolved **open questions** (he cannot ask the user directly), relay them to the user, append answers to the transcript, and re-delegate Rex to finalize. **Before any round ≥ 2 of this loop, run `check_redelegation.py` on Rex's returning handoff** (`--ledger .docs/{project-name}/implementation/gates.jsonl`) — it may halt sooner than the 2-round bound (§2) on an environment/credentials/dependency wall. Loop until `requirements.md` is complete.
...
- **Workflow**:
  ...
  4. Read Aria's returned handoff (validated per §1).
...
- **Workflow**:
  1. **Supersession-annotation lint (mechanical pre-step)**: before the adversarial pass, run `python {PLUGIN_ROOT}/pipeline-tools/scripts/tool_registry.py for --lane bgpdd-plan --phase 2.5` and execute the `check_coverage` line it prints.
  ...
  5. Send the findings to **Aria** as ONE revision round (see §2 — this bound is deliberately tighter than DDD's 3-cycle bound). **Before this round, run `check_redelegation.py` on Aria's Phase 2 handoff** (`--ledger .docs/{project-name}/implementation/gates.jsonl`) — a round-2 delegation regardless of the 1-round cap, so it is still subject to §2's environment/credentials/dependency halt. Unresolved Blockers escalate to the user at the Iteration Checkpoint.
...
- **Workflow**:
  ...
  4. Read Alex's returned handoff (validated per §1).

### Phase 3.5: Coverage & Acceptance Lint Gate (Orchestrator)
- **Delegated Agent**: None — the Orchestrator performs this check directly.
- **Workflow**:
  1. Run `python {PLUGIN_ROOT}/pipeline-tools/scripts/tool_registry.py for --lane bgpdd-plan --phase 3.5` and execute the `check_coverage` line it prints.
  ...
  3. On exit 1 or 2, re-delegate to **Alex** (a fresh delegation) quoting the exact `uncovered` IDs **and** the `lint_failures` entries, plus `warnings` (or the `error` message) — subject to the existing 2-round auto-fix bound. **Before delegating round ≥ 2, run `check_redelegation.py` on Alex's returning handoff** (`--ledger .docs/{project-name}/implementation/gates.jsonl`) — it may halt sooner on an environment/credentials/dependency wall. If unresolved after 2 rounds, halt and surface to the user. After each fix, re-run step 1 to verify.
  4. **Lint the acceptance matrix's structure** — the same gate script, structure mode, no results file yet because nothing has been executed: execute the `check_acceptance_suite` line from the same phase `3.5` listing step 1 printed.
     Exit 0 = structurally sound. Exit 1 = the JSON names the defect (the enumerated defect classes are `check_acceptance_suite.py`'s, in `{PLUGIN_ROOT}/pipeline-tools/SKILL.md`). Exit 2 = usage or an unparseable matrix. `--requirements` additionally gates the FR→scenario link — every Must-Have `FR`/`NFR` cited by at least one scenario heading, each gap landing in `lint_failures` as `fr-scenario-coverage`. Re-delegate to **Alex** quoting the exact arrays, under the same 2-round bound as step 3, re-running after each fix. **Before delegating round ≥ 2 of this loop, run `check_redelegation.py` on Alex's returning handoff** (`--ledger .docs/{project-name}/implementation/gates.jsonl`) — same as step 3.
...
- **Game tape — deliberate divergence (convention #8) from the skeleton's "once per run" reduction**: that reduction holds only for a lane that persists state exactly once; this pipeline persists state **here** (below) and again at Phase 4, so it checkpoints the game tape at both points, not once. Mechanics (file, heading, cap) are the skeleton's, unchanged — see each write below.
...
- **Capability check (both paths, step 3).** For every distinct `[vs:<surface>]` in the plan, confirm the capability that surface's evidence requires actually exists here, exercising each once: browser automation for `[vs:ui]`/`[vs:web+api]`, device or agent access for `[vs:rmm]`, a sink client for `[vs:fn]`, an out-of-process HTTP client and a Python 3 interpreter throughout.
  - **A missing one does NOT halt planning.** Follow the *request now, block at the evidence boundary* ladder in `runtime-evidence`: ask the user for it immediately and specifically (they can install while planning finishes), record it in the same action — run `python {PLUGIN_ROOT}/pipeline-tools/scripts/tool_registry.py for --lane bgpdd-plan --phase 3.6` and execute the `update_state` line it prints for **step 3** — append this write's game-tape checkpoint (`.docs/{project-name}/implementation/game-tape.md`, per the divergence above), and carry on to Phase 4. The blocker travels to `/bgpdd-build` in `orchestrator-state.json`, where Phase 0 re-checks it and the milestone-close rule refuses to let it be forgotten. **Checking early is what buys the user their parallel install time**, not a gate looking for somewhere to stop.
...
- **Persist it so `/bgpdd-build` Phase 0 resolves it instead of re-authoring it**: execute the `update_state` line for **step `-`** in the same phase `3.6` listing — the Tier-1 recipe's path on the brownfield route, the Tier-2 file's on the greenfield one. Append this write's game-tape checkpoint too (per the divergence above) — even on the no-blocker path, this command still persists state.
...
- **Workflow**:
  1. Write this run's game-tape checkpoint at the skeleton's default cadence and cap — at most 10 bullets, heading `## bgpdd-plan — [date]`. This is the run's *second* checkpoint, not its only one — see Phase 3.6's divergence note; the skeleton's "once per run" default does not hold here.
  ...
  3. **State Persistence**: Before concluding, write orchestrator state via `update_state` — never hand-edit JSON. Run `python {PLUGIN_ROOT}/pipeline-tools/scripts/tool_registry.py for --lane bgpdd-plan --phase 4` and execute the `update_state` line it prints. Schema authority is `update_state` (schema version string `"1"`). Its `--init` may already be a no-op here (Phase 3.6 writes the state file first when it records a manifest or a capability blocker) — the warning is expected, and the entries Phase 3.6 wrote are preserved. If Python is unavailable: HALT and surface the missing interpreter.
