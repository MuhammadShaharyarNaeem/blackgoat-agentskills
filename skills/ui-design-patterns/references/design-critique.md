...
Append to the same review report file, inside your `## Review:` block (format owned by code-review-and-quality), as a `### Design Critique` subsection: the specificity verdict, the heuristic table, then findings grouped by severity — every finding with a file/surface, the evidence, and one concrete fix. **The evidence line does NOT go in that subsection** — `check_commit_gate.py` closes a `## Review:` section at any level-2-to-6 heading, so anything under `### Design Critique` is outside the section the gate reads. Put it with the `**Verdict:**` line at the top of the `## Review:` block, per the report template. On a rendered run, the evidence line is a `**Rendered evidence:** <path>[, <path>]` line (bolded — the exact shape the report template in `code-review-and-quality` owns) citing the screenshot/computed-value files you saved under `.docs/{project-name}/implementation/evidence/review/` (a markdown image ref `![...](path)` is the accepted alternative) — this exact token is machine-read by `check_commit_gate.py --require-rendered-evidence`, no variants. `{PLUGIN_ROOT}/pipeline-tools/SKILL.md` is the grammar's single authority; this section cites it rather than restating it. On a source-only run, use the header form from §1.3 instead (see there for what it does and does not satisfy). Summarize only the verdict + Critical/Important counts in your `<handoff>`.
