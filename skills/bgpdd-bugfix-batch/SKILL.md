...
3. **Gate, not prose (convention #9)**: run `python {PLUGIN_ROOT}/pipeline-tools/scripts/tool_registry.py for --lane bgpdd-bugfix-batch --phase 4` and execute the `check_batch_close` line it prints. Verifies every bug in the state's `bugs` list is terminal (`MERGED` — with its own ledger's `check_commit_gate.py` PASS carrying `--commit` — or `DROPPED-PLAN`), every bug's recorded worktree path no longer exists on disk, `{batch-root}/gates.jsonl`'s own hash chain is intact, and `batch.md`'s `## Final` table names every bug with a status-appropriate row. Exit 0 = closed, and is itself the write §1 names for `{batch-root}/gates.jsonl` (no other script appends there). Exit 1 = **BLOCK**; the JSON names the open bug, the leftover worktree, the broken chain or the table mismatch — fix it and re-run. Never hand-close a batch this gate has not PASSed.
