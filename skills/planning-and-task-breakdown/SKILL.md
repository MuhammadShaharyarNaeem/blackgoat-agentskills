...
**Baseline Reconciliation table format (this skill owns it; the duty lives in the planner persona's §4).** A `## Baseline Reconciliation` section at the top of `acceptance-matrix.md`, one row per in-scope baseline case:
...
- [ ] `acceptance-matrix.md` exists, derived from `requirements.md` not the task list; every Must-Have FR/NFR in at least one scenario (`fr-scenario-coverage` lint)
