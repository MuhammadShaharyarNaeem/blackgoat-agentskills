...
- **Handoff validation, bound to this pipeline's returns.** Contract §1 requires every returned `<handoff>` to be validated before you act on it. Save each to a file and run `python {PLUGIN_ROOT}/pipeline-tools/scripts/check_handoff.py --handoff <that file> --persona <rex|aria|alex> --repo . [--repo <dir> ...] --since <the sha HEAD held when you launched that delegation> --milestone "<phase>" --ledger .docs/{project-name}/implementation/gates.jsonl` on all of them: Rex (Phase 1 Step B, and each re-delegation round of its loop), Aria (Phase 2, and her Phase 2.5 revision round), Alex (Phase 3, and each Phase 3.5 re-delegation round). **`--since` is always passed**; add `--fix-round` on a remediation return. Exit 0 = proceed. Exit 1 = the agent's contract was not met — return to that same agent, counting the round its own loop's bound counts. Exit 2 = usage or an artifact defect — fix the invocation or the artifact, not the agent, before re-running.
- **Run log** (Contract §4's obligation, bound here; CLI contract in `{PLUGIN_ROOT}/pipeline-tools/SKILL.md`). Per delegation return — Rex (Phase 1 Step B), each Scout (Phase 2 split), Aria (Phase 2 and her one revision round), Alex (Phase 3 and each coverage-fix round): `python {PLUGIN_ROOT}/pipeline-tools/scripts/record_run.py --log .docs/{project-name}/implementation/run-log.jsonl --pipeline bgpdd-plan --phase "<phase>" --event delegation --agent <name> --model <tier> --unit <the artifact the delegation produced, e.g. requirements.md> …` (`--model` is mandatory on a delegation record — exit 2 without it; name the tier the delegation actually ran at, not the one recommended; a non-Claude runtime id also needs `--tier <haiku|sonnet|opus|fable>`). Add `--rounds <this artifact's round number>` on a re-delegation, plus a token measurement — `--tokens-in`/`--tokens-out` (or `--from-json <the completion payload>`) from the runtime's completion notification, or `--tokens-unavailable "<runtime>: <reason>"` when it exposes none — which is now equally mandatory, and `--status` from the handoff. Into Phase 4's game tape: `python {PLUGIN_ROOT}/pipeline-tools/scripts/summarize_run.py --run-log .docs/{project-name}/implementation/run-log.jsonl --ledger .docs/{project-name}/implementation/gates.jsonl --markdown`.
