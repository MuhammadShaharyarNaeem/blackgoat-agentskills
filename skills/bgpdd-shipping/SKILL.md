...
4. **Ship-decision entry ticket**: Require `.docs/{project-name}/implementation/ship-decision.md` from build Phase 5 (Dep's prep GO/NO-GO). Gate it mechanically:
   ...
   Exit 0 = prep GO present — proceed. Exit 1 = NO-GO or incomplete decision — HALT. Exit 2 = structural/missing. **If Python is unavailable: HALT** and surface the missing interpreter — no manual open-and-read substitute.
...
- **Ship-decision exit ticket (Dep)**: after Vera/Cipher's `check_agent_report` gates pass, gate Dep's refreshed ship-decision before Step 4:
  `python {PLUGIN_ROOT}/pipeline-tools/scripts/check_ship_decision.py --report .docs/{project-name}/implementation/ship-decision.md --require-go`
  Exit 0 = final GO — proceed. Exit 1/2 = BLOCK. **If Python is unavailable: HALT.**
