...
- **Run log** (Contract §4's obligation, bound here; CLI contract in `{PLUGIN_ROOT}/pipeline-tools/SKILL.md`). Per delegation return — each Scout (Phase 2 research), Alex (Phase 2 and each coverage-fix round): run `python {PLUGIN_ROOT}/pipeline-tools/scripts/tool_registry.py for --lane bgpdd-lite --phase '*'` and execute the `record_run` line it prints (`--model` is mandatory on a delegation record — exit 2 without it; name the tier the delegation actually ran at, not the one recommended; a non-Claude runtime id also needs `--tier <haiku|sonnet|opus|fable>`). Add `--rounds <this artifact's round number>` on a re-delegation, plus a token measurement — `--tokens-in`/`--tokens-out` (or `--from-json <the completion payload>`) from the runtime's completion notification, or `--tokens-unavailable "<runtime>: <reason>"` when it exposes none — which is now equally mandatory, and `--status` from the handoff. Into Phase 3's game tape: the `summarize_run` line from that same listing.
...
- **Tier-1 drift, brownfield only (mechanical, convention #9)** — the consumer half of the provenance contract `bgpdd-discovery` §1 states, run once the `{feature}` id is confirmed: run `python {PLUGIN_ROOT}/pipeline-tools/scripts/tool_registry.py for --lane bgpdd-lite --phase 0` and execute the `check_tier1_provenance` line it prints. Drift warns, never halts: relay the artifact, its stamped sha and current HEAD, and let the user choose between planning against this map and re-running `/bgpdd-discovery`. `--allow-drift "<reason>"` records their explicit word — never supply it yourself.
...
- **Workflow**:
  ...
  5. **Initialize state the moment the user confirms — deliberately earlier than the Phase 3 write this replaces (convention #8).** Record the artifacts that do not yet exist as `null`. Why here and not Phase 3: `references/lite-rationale.md` § Initializing state at Phase 1.
     Run `python {PLUGIN_ROOT}/pipeline-tools/scripts/tool_registry.py for --lane bgpdd-lite --phase 1` and execute the `update_state` line it prints.
...
- **Workflow**:
  ...
  4. Read Alex's returned handoff, then **validate it before acting on it** (Contract §1's rule, bound here to this lane's returns — Alex, and each Scout below): save it to a file, then run `python {PLUGIN_ROOT}/pipeline-tools/scripts/tool_registry.py for --lane bgpdd-lite --phase 2` and execute the `check_handoff` line it prints. **`--since` is always passed**; add `--fix-round` on a coverage-fix re-delegation. Exit 1 = his contract was not met — back to Alex under §2's 2-round bound, not on to the coverage gate.
...
- **Workflow**:
  1. Run `python {PLUGIN_ROOT}/pipeline-tools/scripts/tool_registry.py for --lane bgpdd-lite --phase 2.5` and execute the `check_coverage` line it prints.
...
- **Workflow**:
  ...
  2. **State Persistence**: Update the state file Phase 1 step 5 initialized — via `update_state.py`, never by hand-editing JSON, and **without `--init`** (this is an update of the artifacts Alex produced, not a fresh file). Schema authority is `update_state.py` (schema version string `"1"`).
     Run `python {PLUGIN_ROOT}/pipeline-tools/scripts/tool_registry.py for --lane bgpdd-lite --phase 3` and execute the `update_state` line it prints.
