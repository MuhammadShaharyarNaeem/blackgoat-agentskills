...
| Fix round (Quinn's failing spec or Luna's design-critique finding returned to you) | Re-fires this section — "already complete" exempts nothing. Run debugging's REPRODUCE → … → VERIFY; step 7 pins the tier: Quinn's named failing spec, or a fresh render of the precise state Luna critiqued — the new screenshot, not a description → report as `<fix_verification>` (shape: base-persona Output Format, with the [UI] delta in the Override block). The Orchestrator will not forward the fix to Quinn without it |
