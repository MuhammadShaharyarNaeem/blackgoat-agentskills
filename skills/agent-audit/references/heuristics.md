...
**Definition**: A skill's documented content must be correct, not merely well-structured. Three checks: (a) Executable samples — every code sample compiles/runs against the real implementation it documents; a sample using an undeclared dependency or omitting required wiring/registration is a FAIL (e.g. a controller using an undeclared `_mediator`; `UseExceptionHandler` without the `AddProblemDetails` it requires). (b) Internal non-contradiction — the skill gives no two conflicting answers to one question, and no sample contradicts the skill's own doctrine (e.g. a ValidationFilter that bypasses the mandated response envelope). (c) Cross-skill contract coherence — when two skills document opposite ends of one wire contract (a backend response envelope and its frontend consumer), both must describe the SAME serialized wire shape and name the serialization policy that produces it (field names, casing, scalar-vs-object), NOT each side's source-language declaration shape; structured value types that must serialize as scalars require an explicit converter — verify the wire representation, never assume it mirrors the source.
- **Why it matters**: the other metrics check structure; none check whether documented content works or agrees with its sibling — a skill can pass every structural metric while shipping code that throws at startup or a consumer that silently misreads the contract (camelCase-vs-PascalCase makes `isSuccess` undefined → every failure treated as success).
