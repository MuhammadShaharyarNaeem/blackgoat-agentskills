...
The contract states the bands compactly (`XS = 1 file · S = 1-2 · M = 3-5 · L = 5-8 · XL = 8+`); this is the same table with worked examples.
...
An agent performs best on S and M, so anything L or larger gets broken down. The break-it-down-further triggers live in the contract's Task Sizing section — they are rules, not rationale.
...
### Corollary 4 — in-process observations

An in-process test host executes real code and produces a real green result, which is what makes it the most convincing wrong criterion available: everything about it is true except that it observed the boundary the requirement is about. That is why the contract states the rule but refuses to carry the framework list — the recognition list and the "what it cannot prove" column live in `{PLUGIN_ROOT}/runtime-evidence/SKILL.md`, which owns the tier ladder, and the mechanical version lives in `check_runtime_evidence.py` with `check_coverage.py`'s plan lint byte-locked to it. Two prose copies of one blocklist drift; one prose owner plus one byte-locked pair does not.
...
## Milestone Domain Homogeneity — Why a Slice Spans a Pair

Supporting the Step 5 **Milestone domain homogeneity** rule and its convention-#8 refinement label:

The build pipeline routes a whole milestone to one specialized builder, so a milestone holding both `[UI]` and `[API]` tasks has no single correct routing and is rejected at build time. The consequence for Step 3's "Slice Vertically" rule is that a vertical slice can no longer *be* a milestone — it becomes a pair of adjacent single-domain milestones. Nothing about the slicing principle changes; only its unit of expression does, which is why the divergence is labelled rather than silently applied. The seam between the pair is not left implicit either: it is the `provides:`/`consumes:` edge, and because the `[API]` milestone's tasks are lower-numbered, the existing `consumes-provides` lint already enforces the direction of that edge without a new check.

## Verification Surface Tags — Why a Missing Tag Halts

Supporting the Step 5 rule "A missing or unknown surface tag is a planning defect":

Halting is deliberate and it is the point — a surface that can be silently omitted is a surface that will be, and the milestone whose evidence requirement nobody declared is the milestone whose claim nobody checks. `[vs:none]` exists so that "nothing is observable" stays cheap to say, but it is an explicit, reviewable claim rather than an exemption: the justification sentence is what a reviewer challenges, and a default would have produced no sentence to challenge.

**Choosing the surface — worked case.** A milestone that adds a database column and a wrapper around it is `[vs:api]` if a client's response shape changes, and `[vs:none]` only if nothing outside the process can tell the difference. The task list looks identical in both cases, which is why the surface is chosen from the requirement: ask what the requirement's grammatical subject *receives*.

## Runtime Exit Criteria and Aggregate Green — Observed Failure

Supporting the Step 5 rules on runtime exit criteria, the `RUNTIME PROBE:` line, and aggregate green:
...
**Why the probe is written at plan time, by the planner.** A probe invented at verification time is invented by the party motivated to soften it. Writing the machine-parseable line into the checkpoint makes the criterion executable by someone other than its author, which is what lets the build pipeline hand it to QA and gate on the resulting capture.

**Why a compile, typecheck, bundle, or source-search command is not a runtime exit criterion.** `tsc --noEmit`, a clean bundle, and a search proving a symbol exists all prove the code was written; none prove it runs. A test-runner command is the subtler case: `dotnet test` proves a suite is green, never that the hosted app emits anything. And in a multi-service estate the start command matters as much as the probe — a probe against a frontend still pointing at shared dev proves nothing about local, which is why every service and its repointed configuration must be named.
...
## Acceptance Matrix — Scope and Derivation Rationale

Supporting the **Acceptance Matrix Output** section of the contract, which carries the grammar and the worked example.

**Why it is a separate artifact.** Milestone evidence proves each brick; only the matrix proves the wall stands, because the journey is ordered and stateful and because the milestone that adds an operation has no reason to exercise its inverse.

**Why it is derived from `requirements.md` rather than the plan.** A matrix derived from the plan inherits the plan's blind spots, and it is written by the party who would have to redo work if it found something. This is the same rule `shipping-and-launch` states for the launch checklist — *"Enumerate from the requirements document, never from the built feature list"* — and the reason is identical.

**Why `Stores` is per-step and plural.** A mapping that shows a green tick, a row in the database, and a paired agent on the device is three claims; reading one and inferring the others is a proxy substitution.

**Why the step grammar is Echo's.** Reusing `GO → DO → ASSERT` keeps this artifact and `QA/manual-testing.md` diffable, so the write-back at the end of shipping is a merge rather than a translation.

**Why every scenario carries a priority.** An unprioritized scenario cannot be filtered honestly — there is no defensible way to decide what a time-boxed run skips — and it is gated anyway, so omitting the tag buys nothing.

**Why `manual` is first-class but expensive.** Device state, an agent console, and a pairing code genuinely cannot be automated, so refusing manual steps would push those assertions out of the matrix entirely. Requiring recorded evidence under `evidence/runtime/` is what keeps a manual step from degrading into an unchecked claim: unevidenced, it reads as NOT RUN and blocks.

**Why reinstall is named explicitly.** Install→uninstall→reinstall is the idempotency case, and it is the one that catches an uninstall that only half-removed; an install/uninstall pair alone passes over that defect.
...
For the same reason, the only sanctioned repair is to **fix the sample**. Restating the invariant beside a contradicting sample leaves both in the plan and lets the builder pick the concrete one; and an acceptance criterion written to match the sample's lossy behaviour promotes the defect into the gate, so the finished work passes by asserting the wrong thing.
...
## Risk Traceability — Rationale

Supporting the Verification checklist row on High-impact risks:

A risk whose only entry is prose in the Risks and Mitigations table is unmitigated, because nothing in the plan fails when the mitigation does not hold. Tracing each High-impact risk to a specific acceptance criterion or checkpoint criterion that would **detect** it converts the mitigation from an intention into something the pipeline can observe going wrong.
