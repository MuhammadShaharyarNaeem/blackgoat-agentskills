...
| changes a **published API contract document** breakingly | carry the `[breaking: …]` line (*Contract-change tags* below) |
| introduces a **feature flag** | be written together with the task that removes it (*Contract-change tags* below) |

#### Contract-change tags

Two obligations the plan carries so a builder is never asked to invent them. Both are lines in the task body, not `**Tags:**` entries, and **neither is checked by any lint** — the finalizing sweep is the only guard.

**`[breaking: <kind> — <consumer impact>]`** is required on every task that authorizes a breaking change to a published API contract document. `<kind>` is one of the seven class names owned by `{PLUGIN_ROOT}/api-contract-evolution/SKILL.md` (§ The Breaking-Change Classes); `<consumer impact>` is one line naming who has to change. The same task also carries the consumer list, the new version, and the deprecation window for the old shape. A breaking change with no `[breaking:]` line is **returned unbuilt as a planning defect** — the builder is instructed never to add the line itself, so an omission here stalls the milestone rather than passing quietly.

**A task that introduces a feature flag is written together with the task that removes it**, in the same planning pass — the rule is owned by `{PLUGIN_ROOT}/feature-flag-patterns/SKILL.md` (§ The removal task is created at creation time). Express the seam in `Boundary contracts:`: the creation task leads with `provides: flag.<name>`, every gated task with `consumes: flag.<name>`, and the removal task with `consumes: flag.<name>` plus whatever the collapsed branch then provides unconditionally. `flag.<name>` is already inside the `consumes-provides` lint's charset, so the **ordering** is enforced; the **existence** of the removal task is not — a flag with no removal task is a planning defect no lint can see.
