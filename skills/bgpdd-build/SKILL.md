...
- **Hydration Phase**: Before Phase 1, read `.docs/{project-name}/orchestrator-state.json` (if it exists) to restore context from the planning phase. The file follows the schema defined in `bgpdd-plan` Phase 4 (fields: `schema`, `project_name`, `feature`, `pipeline`, `branch`, `milestone_cursor`, `artifacts`, `blockers`, `updated`). At hydration, once the working branch is established (see Git Workflow below), record it in the `branch` field.
  ...
  - **`plan.md` is the single authority on what to build next; `milestone_cursor` is only a resume hint.** Per the Orchestrator Contract §4, the next milestone is **the first unchecked (`[ ]`) milestone in `plan.md`'s own top-to-bottom order** — always re-derived by reading the plan, never taken from the stored cursor and never advanced by stepping forward from it. Re-derive at hydration AND after every edit to `plan.md`, including edits you make mid-run. If the plan holds unchecked work *earlier* than the stored cursor — a remediation milestone inserted behind it, a milestone reopened by a later finding — the cursor is **stale**: reset it to that earlier milestone and tell the user before proceeding. Reading the cursor as "resume forward from here" silently skips everything inserted behind it, which makes a whole round of corrective work inert while every artifact on disk still looks correct.
  - After each milestone completes its Build→Test→Review→Refactor cycle **and you have personally observed its runtime exit criterion** (Orchestrator Contract §4, *green is not evidence* — a milestone does not close on returned `COMPLETE` statuses), update the state file: set `pipeline` to `"bgpdd-build"`, re-derive `milestone_cursor` from `plan.md` as above (or `null` when no unchecked milestone remains), append any unresolved Critical/Important findings, BLOCKED verifications, and named proxy substitutions to `blockers`, and append this milestone's evidence to `game-tape.md` (Phase 6). A milestone may not be marked `[x]` while its `blockers` entries stand.
- **Git Workflow**:
  ...
  - **The commit gate is mechanical: its preconditions are artifacts you read off disk, not a judgement you form.** Before writing the commit, open both files and confirm:
    a. `.docs/{project-name}/implementation/review-report.md` — read the latest `**Verdict:**` line under the latest `## Review:` heading for the milestone (values: `Approve` or `Request Changes`). The gate passes only if that line reads `Approve`, AND the review postdates Mason's last diff for this milestone. A verdict written before the most recent fix reviewed different code and does not count.
    b. `orchestrator-state.json` — the `blockers` array holds **no** entries scoped to this milestone.
    If either check fails you do not commit; route back into the Phase 3 step 4 cycle. There is no "commit now, the remaining finding is minor" — that judgement is exactly what this gate exists to remove. Note that the prose rules which should already have prevented this (Orchestrator Contract §4, *green is not evidence* and the append-only `blockers` ledger) were in force during an observed run in which three consecutive milestones were committed over standing `Request changes` verdicts. Nobody denied the rules; they were simply never converted into a file you had to open. This is that conversion — perform the two reads, or you have not run the gate.
...
- **Workflow**:
  1. Pick the next milestone by **re-reading `implementation/plan.md` and taking the first unchecked (`[ ]`) milestone in the file's own order** — not the stored `milestone_cursor`, and not the one after the milestone you just finished (see the Hydration Phase rule in §1). Do this every time you enter Phase 1, not once per run.
  2. Delegate the milestone to a **single Mason** agent, who builds its tasks in plan order. **The milestone is the unit of delegation — do NOT fan the milestone's tasks out to parallel Mason agents**, even when their `Dependencies:` fields show no ordering constraint between them. Parallel builders committing to the shared working branch move HEAD under their siblings, and every verification round then re-verifies the whole moving diff instead of one task's changes — multiplying token cost for a wall-clock gain the pipeline doesn't need. One Mason builds; one Quinn/Luna round verifies the milestone's diff once. **CRITICAL CONTEXT HANDOFF**: copy the exact text of the active milestone from `plan.md` into the delegation prompt so he knows exactly what to build.
...
- **Workflow**:
  ...
  4. If Luna's or Cipher's handoff flags "Critical" or "Important" blockers (including any Cipher vulnerability finding per step 1b), delegate a fresh Mason agent to resolve them — and treat that remediation as a **cycle, not a tail**. Mason's fix re-enters the pipeline: back to **Phase 2**, where a fresh Quinn re-tests the fix (brief her with the exact findings and Mason's remediation `<changed_files>`), then back to **Phase 3**, where a fresh Luna re-reviews the **remediation diff itself** and records a new verdict in `review-report.md`. You exit the cycle only when Luna's verdict on the CURRENT diff is `Approve` — never on Mason's assertion that he fixed it, and never by carrying a verdict written against the pre-fix code. Bound the cycle with the Orchestrator Contract's 2-round autonomous-rejection rule: if the findings survive two full cycles, HALT and surface the milestone, both remediation attempts, and the standing findings to the user. The observed failure this exists to stop is Mason → Quinn → Luna → Mason-fix → **commit**, where the fix itself was never tested and never reviewed, and where one such "fix" closed a missing-route finding by adding the placeholder route the rule forbade.
