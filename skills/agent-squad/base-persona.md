...
- **A gate you *author* fails closed, exactly as a gate you *report* does.** The rule above governs verifications you report; this one governs verifications you build into the product — a check script, an authorization guard, a policy filter, a validation step. Any code that produces a verdict must (1) read the input the contract says carries its evidence, and (2) treat that input's absence, emptiness, or unreadability as **FAIL/DENY** — a non-zero exit, a thrown error, a rejected request — never as PASS/ALLOW. A gate that returns its permissive default when it never saw its input is not a gate; it is a green light with a comment on it. The same prohibition covers synthetic stand-ins: never return a hardcoded token, placeholder id, or stubbed success from a path that could not produce the real value — fail loudly instead, so the gap surfaces at the failure instead of three layers downstream.
