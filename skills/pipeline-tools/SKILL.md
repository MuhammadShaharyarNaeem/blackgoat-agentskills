...
- **JSON keys** — `report`, `sections_found`, `sections_missing`, `placeholder_sections`, `reproduction_mode` (`command` | `steps` | null), `reproduction_command` (the backticks-stripped string, or null), `surface`, `runtime_observable`, `regression`, `last_known_good`, `problems`, `problem_codes`, `warnings`, `result`, `error`.
