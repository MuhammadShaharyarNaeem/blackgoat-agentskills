...
- **`Mode` fails closed.** `manual` demands evidence, so an unrecognized value must not be the cheaper option: a cell that is present but is neither `auto` nor `manual` — `Manual!`, `semi`, `manual (device)` — is gated **as manual**, with a warning saying it was gated that way because the mode was unrecognized rather than because it declared manual. A blank cell or an absent column still defaults to `auto`; that boundary is deliberate — it decides which evidence grammar a step owes, not whether it owes evidence at all, since every `auto` step (typed, defaulted, or column-absent) now owes a `evidence/runtime/` capture with a matching sidecar of its own (see below). Before this, `semi` was silently read as `auto` and a device step citing nothing passed green — verified against the previous revision, not assumed.
...
Verifies the matrix was **executed and evidenced** — never that a scenario is the *right* scenario, that its ASSERT column asserts the right thing, or that preconditions and ordering were honored; authoring is Alex's judgment and stays reviewable prose. It **never opens** a cited capture beyond an existence check — whether the capture is honest (out-of-process transport, freshness, response keys) is `check_runtime_evidence.py`'s job, and running both is the point. **Every `PASS` whose Mode is not explicitly `manual`** — an `auto` step, whether typed, defaulted from a blank cell, or defaulted from a missing `Mode` column — **must cite a capture under `evidence/runtime/`** carrying a `.meta.json` sidecar whose `capture_sha256` still matches the capture's bytes: `auto_step_uncaptured` (no citation), `auto_step_sidecar_missing`, `auto_step_capture_hash_mismatch`. Two typed lines no longer close an acceptance suite at exit 0.
