...
17. **Apply Metric 15 (Skill Content Validity & Cross-Skill Contract Coherence)**: Verify the skill's documented content is correct, not just well-structured — (a) every code sample compiles/runs against the real implementation (no undeclared dependencies or missing wiring/registration); (b) the skill is internally non-contradictory and no sample violates its own doctrine; (c) when two skills document opposite ends of one wire contract, both describe the SAME serialized wire shape and name the serialization policy (field names, casing, scalar-vs-object), not each side's source-language declaration shape; (d) **trigger phrasing matches intended cadence** — a continuous obligation phrased as a completion-conditioned one teaches deferral. Flag any rule whose trigger clause ("when finished", "at the end", "once complete") gates an action that should be happening throughout. Observed cost: `base-persona.md`'s "When your task is complete: save your final output" was structurally correct and destroyed two complete multi-call agent runs, because it taught *when* to write, not merely *what* to write.
