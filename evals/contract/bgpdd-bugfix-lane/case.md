...
Probed 2026-09-03 in this environment: a headless `claude -p … --permission-mode acceptEdits` run auto-approves read-only commands (`echo`, `git --version`, `python --version`) but answers `This command requires approval` to any write-effect command — a gate script writing its ledger, `python -c` writing a file, `node -e` writing a file — and a non-interactive run has nobody to approve it. Run 1 of this case (316 s) reproduced exactly that: the Orchestrator wrote `bug-report.md`, hit the denial on `check_bugfix_intake.py`, and **HALTed at Phase 0 step 3 rather than declare the PASS or delegate ungated** — correct lane behaviour, archived as INFRA. With `--allowedTools` naming the shell, file and delegation tools, the same probes ran. A handoff that still shows denied tool calls is an INFRA finding per `agent-audit` Metric 21: classify it, fix the harness, re-run; never read it as evidence about the spine.
