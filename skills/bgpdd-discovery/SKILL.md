...
- **Workflow**:
  ...
  8. **Provenance gate (Orchestrator, before closing the phase)**: run `python {PLUGIN_ROOT}/pipeline-tools/scripts/check_tier1_provenance.py --summary-root .docs/summary --repo <name>=<path> [--repo ...] --warn-on-drift --ledger .docs/summary/gates.jsonl`, passing one `--repo` per in-scope repo from the Target Scope. Exit 1 means Iris's `context.md` header is unstamped, undated, or names a sha that resolves to nothing — return it to her rather than stamping it yourself; §1's stamp must be read from the repo at write time, and a stamp you supply is exactly the recalled value the rule forbids. Drift exits 0.
