...
3. **The command MUST exit non-zero while the bug is present** — Phase 4's gate rejects a RED that succeeded, so HTTP probes use `curl --fail` (`-f`), which exits 22 on 4xx/5xx (a bare `curl` exits 0 on a 500). Given steps rather than a command, she writes the smallest failing test reproducing the reported behaviour and captures **that** run identically.
...
1. Brief her with the builder's `<changed_files>` and `<consumers>`, the root cause, **the RED path she must confirm was not modified**, Quinn's evidence, and `{bugfix-root}/review-report.md` as her report path.
