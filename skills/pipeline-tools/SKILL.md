...
- **Self-test** — **46** cases, each against a real temp git repo, including an end-to-end capture written by the real `run_quiet.py` and the glob matcher's own unit table (Windows separators included). Depth (the two divergences, the frozen status-letter rule, the copy-not-import decision, the case inventory): `references/check_quick_close.md`.
...
- **JSON keys** — `report`, `section`, `verdict`, `checks`, `passed`, `failed`, `blocked`, `not_run`, `unevidenced`, `uncaptured`, `capture_disagrees`, `capture_problems` (`[{check, problem, detail}]`), `capture_inputs`, `allow_uncaptured`, `critical_findings`, `warnings`, `result`, `error`. **Problem codes** (in `capture_problems[].problem`): `check_uncaptured`, `check_capture_disagrees`.
- **Exit codes** — **0** verdict exactly `Pass`, at least one check line, every line `PASS`, evidenced and captured, zero Critical findings; **1** any other verdict (an unparseable latest verdict fail-safes to no-verdict), a non-empty `failed`/`blocked`/`not_run`/`unevidenced`/`uncaptured`(unwaived)/`capture_disagrees`, a standing Critical, or zero check lines; **2** usage error, missing/empty/unreadable report, or no `## ` section carrying a `**Verdict:**` line.
- **Migration** — a report authored under 2.3.0 or earlier fails with `check_uncaptured` on every executed line. The fix is to re-run each check through `run_quiet.py --capture` and cite the artifact; producer-side grammar lives in `agents/cipher.md`, `agents/vera.md` and `shipping-and-launch/SKILL.md`.
- **Self-test** — **38** cases. Depth (check-line grammar, producer-side pointers, fixtures): `references/check_agent_report.md`.
...
- **JSON keys** — `red`, `green`, `green_runs`, `red_result`, `green_results`, `command`, `problems`, `problem_codes`, `warnings`, `result`, `error`. Per capture: `path`, `role`, `exists`, `is_capture`, `sidecar`, `sidecar_present`, `sidecar_capture_sha256_ok`, `sidecar_body_agrees`, `exit_code`, `argv`, `finished`, `problems`, `problem_codes`.
- **Problem codes** — `capture_missing`, `not_a_capture`, `sidecar_missing`, `sidecar_hash_mismatch`, `sidecar_body_disagrees`, `capture_header_missing`, `sidecar_no_exit_code`, `sidecar_no_argv`, `red_exit_zero`, `green_exit_nonzero`, `command_mismatch`, `green_not_newer`, `timestamp_unparseable`, `green_runs_short`.
...
- **Self-test** — **31** cases, including an end-to-end pair produced by the real `run_quiet.py`.
...
- **Self-test** — **59** cases across four suites: every phase boundary for both lanes, feature vs standalone roots, FAIL and stale records blocking, a report edited after intake PASS, a RED that exited 0, runtime-observable requiring the runtime gate, the deliberate skip (a builder delegation recorded while the route gate never ran), both lanes complete at exit 3, and the `detect_stack.py` defaults reaching the Phase 0 and Phase 3 actions (with both fallbacks). Depth (phase tables, the exit-code design, what the other lanes would need): `references/pipeline_driver.md`.
...
- **Breaking `kind`s** — `path_removed`, `operation_removed`, `response_status_removed`, `response_field_removed` (a rename shows as this plus an additive `response_field_added`; the gate never guesses the pairing), `type_changed`, `required_request_field_added` (a new required request property, an existing one added to `required`, a new required parameter, an existing parameter turned required, or `requestBody.required` flipped true), `enum_narrowed` (a value lost, **or** an enum introduced where the base accepted any value). Additive `kind`s, recorded and never gating: `path_added`, `operation_added`, `response_status_added`, `response_field_added`, `optional_request_field_added`, `parameter_added`, `enum_widened`.
- **JSON keys** — `base`, `head`, `result` (`PASS` | `ALLOWED` | `FAIL` | `ERROR`), `breaking`, `additive` (both arrays of `{kind, path, detail}`), `allowed_by`, `error`. `breaking` stays fully populated under a waiver — `result: "ALLOWED"` is what says it was waived.
- **`$ref`** — resolved **one level** over any local `#/...` pointer (`#/components/schemas/X` primarily). A resolved schema's own `$ref`s are left alone, which is what makes a self-referential component terminate; a two-deep chain compares symmetrically and reports nothing. `allOf` members are merged; `oneOf`/`anyOf` are not decomposed.
...
- **Exit codes** — **0** every difference is additive, or a breaking diff carried a non-empty `--allow-breaking`; **1** a breaking diff; **2** a missing `--base`/`--head`, a document that is missing, unreadable, not valid JSON, YAML outside the supported subset, or not a mapping at the top level, or an empty `--allow-breaking`.
- **Self-test** — **38** cases. Depth (the per-class detection rules, the `$ref` and YAML scope limits, what the waiver does and does not buy): `references/check_openapi_diff.md`.
