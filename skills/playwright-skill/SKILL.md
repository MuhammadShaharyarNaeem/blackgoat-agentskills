...
2. **Write scripts to /tmp** - NEVER write test files to skill directory. Write test files to the system's temporary directory:
- **macOS/Linux**: `/tmp/playwright-test-*.js`
- **Windows**: `$env:TEMP\playwright-test-*.js`
  (typically `C:\Users\{user}\AppData\Local\Temp\`)

In Node.js scripts, use `os.tmpdir()` for cross-platform
temp directory resolution:
```javascript
const os = require('os');
const path = require('path');
const scriptPath = path.join(os.tmpdir(), 'playwright-test-page.js');
```
...
# Windows PowerShell:
Push-Location $SKILL_DIR; node run.js /tmp/playwright-test-page.js; Pop-Location
```
...
For robust E2E testing patterns, checklists, and advanced structural strategies, see the [Implementation Playbook](resources/implementation-playbook.md).
