...
- **Run log** (Contract §4's obligation, bound here; CLI contract in `{PLUGIN_ROOT}/pipeline-tools/SKILL.md`). Per delegation return — Vera (Step 2 Stage 1), Cipher and Dep (Step 2 Stage 2), the fresh Quinn (Step 3's acceptance re-execution), the fresh Dep (Step 5.5), Forge (Step 7, both the propose and the apply delegation), and every fix-and-reverify round in between: run `python {PLUGIN_ROOT}/pipeline-tools/scripts/tool_registry.py for --lane bgpdd-shipping --phase '*'` and execute the `record_run` line it prints (`--model` is mandatory on a delegation record — exit 2 without it; name the tier the delegation actually ran at, not the one recommended; a non-Claude runtime id also needs `--tier <haiku|sonnet|opus|fable>`). Add `--rounds <this checklist area's round number>` on a re-delegation, plus a token measurement — `--tokens-in`/`--tokens-out` (or `--from-json <the completion payload>`) from the runtime's completion notification, or `--tokens-unavailable "<runtime>: <reason>"` when it exposes none — which is now equally mandatory, and `--status` from the handoff. **This is what makes Step 6.5's whole-epic roll-up true.**
...
2b. **Stamp the pipeline the moment step 2 accepts the state (convention #9)**: run `python {PLUGIN_ROOT}/pipeline-tools/scripts/tool_registry.py for --lane bgpdd-shipping --phase step-0` and execute the `update_state` line it prints — no `--ledger`, a state writer and not a gate (the Gate conventions above bind the path for gates only). Nothing else here writes the value step 2 and `bgpdd-build` §1's re-entry rule both read, so without this write step 4's resume branch and that re-entry are unreachable. **Branch on the value you read in step 2, never on the one you just wrote.** **Tape it now (Orchestrator Contract §4 — a checkpoint at every state persistence, not deferred to Step 6.5):** append one bullet to `.docs/{project-name}/implementation/game-tape.md`'s `## bgpdd-shipping — [date]` section (create the file/heading if this run's first tape write) naming this stamp and whether it was a fresh run or a resume.
2c. **Tier-1 drift — the consumer half of `bgpdd-discovery` §1's provenance contract**: the `check_tier1_provenance` line from that same step-0 listing. Drift warns, never halts: relay the named artifact, its stamped sha and current HEAD to the user, and pass `--allow-drift "<their reason>"` only on their explicit word, recording that reason.
3. **Prerequisite Gate (mechanical, convention #9)**: do not eyeball `plan.md`. Run the `next_milestone` line from that same step-0 listing.
...
4. **Ship-decision entry ticket**: Require `.docs/{project-name}/implementation/ship-decision.md` from build Phase 5 (Dep's prep GO/NO-GO). Gate it mechanically — **the flags depend on the `pipeline` value you just hydrated in step 2**, because entry ticket and exit ticket are the same mutable file:
   - **Fresh run** (`pipeline` was `"bgpdd-build"`): the file still holds build's prep verdict, so require the GO: the step-0 listing's `check_ship_decision` line that carries `--require-go`.
     Exit 0 = prep GO present — proceed. Exit 1 = NO-GO or incomplete decision — HALT. Exit 2 = structural/missing.
   - **Resume** (`pipeline` was `"bgpdd-shipping"`): Stage 2 Dep has already rewritten this file, so its content is now an *exit* verdict, not an entry one. Run the **structural check only** — the step-0 listing's other `check_ship_decision` line, the one without `--require-go`.
...
5. **Blockers Ledger Gate**: mechanical — no open-and-read substitute. Run the `check_blockers` line from that same step-0 listing.
...
- **Handoff Gate (mechanical, Orchestrator Contract §1) — before any report gate below.** Save each returned `<handoff>` to a file and validate it before acting on it:
  run `python {PLUGIN_ROOT}/pipeline-tools/scripts/tool_registry.py for --lane bgpdd-shipping --phase step-3` and execute its three `check_handoff` lines — one each for Vera (`--persona vera`, `--since` the sha HEAD held when you launched Stage 1), Cipher and Dep (`--persona cipher` / `--persona dep`, `--since` the sha HEAD held when you launched Stage 2).
  Add `--fix-round` on whichever of the three is a remediation return. Exit 1 = the agent's contract was not met — back to that agent, counting the round under the Fix-routing bound below.
- **Report Gate (mechanical)**: a "pass" in Vera's or Cipher's handoff is a claim; the report file is the evidence. Run it once per report: the step-3 listing's two `check_agent_report` lines, one for `verification-report.md` and one for `security-report.md`.
...
- **Runtime Evidence Gate (mechanical)**: `check_agent_report` verifies Vera's verdict is backed by evidenced check lines with exit codes — it does **not** verify that anything was ever started. Run the step-3 listing's `check_runtime_evidence` line once Stage 1's report is in hand, and again after any fix round.
...
- **Acceptance Suite Re-execution Gate (mechanical)**: **re-reading build's `acceptance-results.md` is not this run** — the walkthrough has to be *executed again* against the code that is about to ship (`references/shipping-rationale.md` § Why the acceptance suite is re-executed rather than re-read). Resolve the matrix path from `artifacts.acceptance_matrix` on the state hydrated in Step 0. **If it is JSON `null`** (a lite-originated epic with no matrix), skip this gate and say so explicitly to the user — do not re-derive from file absence. Otherwise:
  ...
  2. She **appends** a new `## bgpdd-shipping — [date]` results section to `.docs/{project-name}/implementation/acceptance-results.md` — append, never overwrite: build's section is the baseline a regression is read against. Every step's evidence is a fresh capture written via the step-3 listing's `run_quiet` line, under `.docs/{project-name}/implementation/evidence/runtime/`, cited inline in the result line's detail field per `{PLUGIN_ROOT}/runtime-evidence/SKILL.md`.
  2b. **Handoff Gate (mechanical, Orchestrator Contract §1).** Save her returned `<handoff>` and validate it before the freshness gate below:
     the step-3 listing's `check_handoff` line for `--persona quinn` (add `--fix-round` on a remediation return). Exit 1 = back to her, counting the round under this gate's own bound below.
  3. Then run the gate with the freshness assertion: the step-3 listing's `check_acceptance_suite` line.
...
- **Routing a shipping finding back to `/bgpdd-build`**: `bgpdd-build`'s hydration accepts `pipeline: bgpdd-shipping` as a re-entry — it resets the state to `bgpdd-build` and records the shipping finding in the game tape, so you no longer have to launder the state to get back in. **Before you hand the user the `/bgpdd-build` instruction, record the finding as a blocker**:
  the step-3 listing's `update_state` line.
  ...
  **Then reopen the milestone the finding lands in — mechanically, before you hand over (convention #9).** Step 0.3 required `DONE`, so every milestone is `[x]` and build's "proceed to the milestone that finding affects" has nothing pending to route to:
  the step-3 listing's `mark_milestone` line.
  **Tape it now (Orchestrator Contract §4 — a checkpoint at every state persistence, not deferred to Step 6.5):** append one bullet to `.docs/{project-name}/implementation/game-tape.md`'s `## bgpdd-shipping — [date]` section (create the file/heading if this run's first tape write) naming the finding, the blocker just recorded, and the reopened milestone.
  The `[x]` comes off with the evidence recorded, so `next_milestone.py` returns that milestone as `NEXT` for build's re-entry rather than `DONE`. Then re-enter shipping at Step 0 — build's re-entry handler (`bgpdd-build` §1) resets `pipeline` back to `bgpdd-build` before it builds, so this hydration reads `bgpdd-build`, not `bgpdd-shipping`, and Step 0.4 takes the **fresh-run** branch, re-requiring `--require-go` (and, at Step 3's exit ticket, `--require-rehearsal`/`--require-baseline` again). **Not the resume branch** — that branch fires only when hydration reads `pipeline: bgpdd-shipping`, i.e. an interrupted shipping session picking back up inside this same pipeline, never a build round trip. You cannot proceed past Step 3 until the Launch Squad is fully green.
...
1. Run `python {PLUGIN_ROOT}/pipeline-tools/scripts/tool_registry.py for --lane bgpdd-shipping --phase step-3.5` and execute the `check_coverage` line it prints.
...
2b. **Handoff Gate (mechanical, Orchestrator Contract §1) — before the report gate below.** Save this Dep's returned `<handoff>` and validate it:
   run `python {PLUGIN_ROOT}/pipeline-tools/scripts/tool_registry.py for --lane bgpdd-shipping --phase step-5.5` and execute the `check_handoff` line it prints (add `--fix-round` on a remediation return). Exit 1 = back to him, counting the round under Step 3's fix-and-reverify bound.
3. **Gate the report (mechanical).** Dep's handoff is a claim; the report is the evidence. Run both, in this order: that same step-5.5 listing's `check_agent_report` line, then its `check_runtime_evidence` line.
...
No delegation, no halt — you perform this step directly, before briefing Forge. Cadence, cap, heading grammar, path and the ledger-citation rule are the skeleton's Game Tape section, unchanged — the default cadence is a checkpoint at every state persistence (Contract §4), so this step is the run's **final** tape write, appending to whatever bullets Step 0.2b and Step 3's re-entry rule already wrote this run under the same `## bgpdd-shipping — [date]` heading, not the sole checkpoint. This pipeline's two additions:
...
2. **Paste the whole-epic run-log roll-up** under the bullets — the epic's only cross-pipeline cost record, and the one place the fired-versus-rubber-stamped column is computed rather than remembered:
   run `python {PLUGIN_ROOT}/pipeline-tools/scripts/tool_registry.py for --lane bgpdd-shipping --phase step-6.5` and execute the `summarize_run` line it prints.
...
- **Workflow**:
  ...
  2b. **Handoff Gate (mechanical, Orchestrator Contract §1) — advisory.** Save Forge's returned `<handoff>` and validate it before HALTing for approval:
     run `python {PLUGIN_ROOT}/pipeline-tools/scripts/tool_registry.py for --lane bgpdd-shipping --phase step-7` and execute the `check_handoff` line it prints for **step 2b** — `--advisory` because this propose round is a recommendation, not a written artifact (Contract §1's `--advisory` bullet). Exit 1 = the proposal's contract was not met — do not HALT for approval on it; return to Forge for a conforming proposal.
  ...
  5b. **Handoff Gate (mechanical, Orchestrator Contract §1).** Save the applying Forge's returned `<handoff>` and validate it before the write-boundary check in step 6:
     the `check_handoff` line for **step 5b** in that same step-7 listing. Exit 1 = back to Forge for a conforming `<changed_skills>` list.
