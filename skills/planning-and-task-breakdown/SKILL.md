...
**A criterion, a dependency identifier, and a prerequisite all name the observable effect the consumer needs — never the existence of the code, module, or decision that will eventually produce it.** No parser catches a violation of this. Five applications:
...
5. **A criterion stays inside the change's control.** A criterion is unusable when its truth depends on something the change does not own. Three recurring forms: **(a) the shape of the diff** — exact insertion/deletion counts, "insertions only", "zero modified lines" — frequently *mechanically unsatisfiable*, since appending to a list whose last element lacks a trailing separator necessarily rewrites that line, so a correct implementation fails its own gate; **(b) the position of a moving external ref** — pin the immutable snapshot id (the commit) the work was actually done against, never a branch name, and never write a criterion whose truth flips when a shared base advances; **(c) a value another party allocates** — an enum ordinal, id, port, or reserved slot is **unreserved until re-read at implementation time**, and never frozen into a criterion as a constant. Constrain the observable effect instead; where a mechanical shape genuinely is the proof, state the exact deviation it must produce.

(Rationale for all five, example pairs, and the resolve-time vs run-time distinction: [deep dive](references/planning-deep-dive.md).)
...
- [ ] No criterion constrains diff shape, a moving external ref, or a value another party allocates (corollary 5) — every enum/list-append criterion checked against the base file's actual trailing-separator state
