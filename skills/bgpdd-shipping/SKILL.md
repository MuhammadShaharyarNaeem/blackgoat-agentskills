...
4. **Blockers Ledger Gate**: this gate is mechanical — its precondition is a file you read off disk, not a judgement you form. Open `.docs/{project-name}/orchestrator-state.json` and read the `blockers` array. If it holds ANY unresolved entries, HALT and list every standing entry verbatim to the user — shipping may not start over an open blocker.
