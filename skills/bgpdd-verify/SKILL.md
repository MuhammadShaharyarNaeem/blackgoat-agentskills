...
- **Workflow**:
  ...
  5. **Lint gate (mechanical)**: `python {PLUGIN_ROOT}/pipeline-tools/scripts/check_acceptance_suite.py --lint-only --matrix .docs/{project-name}/acceptance-matrix.md` — exit 0 required before Phase 2; on exit 1/2 fix the named arrays and re-run (2-round bound, then halt and surface).
...
- **Workflow**:
  ...
  5. **Re-entry: this state file is terminal by design.** `bgpdd-build` and `bgpdd-shipping` hydration reject `pipeline: bgpdd-verify`, and that rejection is the intent, not a gap: this lane produces findings, never fixes, so the way out of a verify run is a fresh `/bgpdd-bugfix` session per defect (step 2), and the way back in is the Phase 0 re-verify shortcut. What must never happen is a later `/bgpdd-plan` or `/bgpdd-lite` run `--init`-merging its own state over this file: an init on the same slug replaces the verify run's `acceptance_matrix` and pipeline value with a plan-derived one, and the durable evidence triplet stops matching the state that points at it. The guard is the slug rule, not vigilance — Phase 0 step 3 defaults this run to `{feature}-verify` and its slug-collision guard HALTs a verify run that lands on a plan/build slug, so a build-lane project and a verify run never share a state path.
